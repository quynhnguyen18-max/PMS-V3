(function(root,factory){
  ...
  const BASE=[
    {id:'f1',thankedByReceiver:true,thankedByManagers:[{name:'Đỗ Quang Huy',dom:'huy.do'},{name:'Phan Anh Tuấn',dom:'tuan.phan'}],employeeId:'e1',cycle:'2026',sender:{name:'Trương Minh Đức',dom:'duc.truong',ini:'TĐ'},date:'05/06/2026',visibility:'manager',body:'Tú xử lý incident production rất chắc chắn, phối hợp rõ ràng với các team liên quan và luôn giữ được sự bình tĩnh khi có áp lực.',cv:['Thực thi xuất sắc','Tinh thần đồng đội']},
    ...
    {id:'f9',thankedByReceiver:true,employeeId:'e12',cycle:'2026',sender:{name:'Đinh Văn Tùng',dom:'tung.dinh',ini:'ĐT'},date:'25/07/2026',visibility:'manager',body:'Bích không chỉ hoàn thành tốt phần kỹ thuật mà còn dành thời gian mentor hai bạn junior lên mid-level.',cv:['Tinh thần đồng đội','Không ngừng học hỏi']}
  ];
  const SENDERS=[
  ...
  /* Dấu tim cảm ơn — chỉ hiện khi trang đã nạp ManagerThanks và đăng ký store bằng
     ManagerThanks.use(). Không có provider (ví dụ khi chạy test ở Node) thì card giữ nguyên
     như cũ, chỉ còn tim của người nhận nếu dữ liệu có sẵn thankedByReceiver. */
  function thanksApi(){
    const api=(typeof globalThis!=='undefined'?globalThis:this).ManagerThanks;
    return api&&api.active&&api.active()?api:null;
  }
  function feedbackCard(item,employee,options={}){
    ...
    const state=thanks?thanks.stateFor(item):{receiver:!!item.thankedByReceiver,managers:[],manager:false};
    const mark=thanks?thanks.markHTML(state,item.sender.name,false,{receiverDomain:employee&&employee.login}):'';
    /* Thanh "Cảm ơn" nằm ở chân card, đúng vị trí và cách thể hiện của màn nhân viên E-04.
       Đã thả tim rồi thì thanh biến mất với riêng người đang xem. Tim của quản lý khác
       vẫn giữ nguyên trong cụm tim và không chặn lượt cảm ơn của người này. */

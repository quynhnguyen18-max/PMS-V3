...
8b. **Questionnaire download:** Nút tải xuống nằm trong page actions, ngay bên trái CTA **Tạo bộ câu hỏi**. Nút mở popup để chọn **Toàn bộ thư viện** hoặc **Từng bộ riêng biệt**; toàn bộ thư viện xuất một CSV UTF-8 tổng hợp mọi bộ người dùng được phép xem, còn chế độ riêng biệt cho chọn một bộ và xuất CSV của bộ đó. Chỉ nút tải ở page actions dùng icon `bx-download` trong nút vuông 34px và hover/focus hiện tooltip **Tải xuống**. CTA chính trong popup lựa chọn tải xuống và nút tải trong popup xem chi tiết từng bộ đều hiển thị đầy đủ icon + text **Tải xuống**. Không thêm icon tải xuống vào từng hàng; cột Chức năng giữ grid 4 ô và rộng 124px như rule chính.

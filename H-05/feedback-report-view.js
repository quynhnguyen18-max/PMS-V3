...
(function(root,factory){
  ...
  // Report entries are visible only after HR releases the selected audience for that recipient.
  function entriesFor(domain,viewer='recipients'){
    ...
    return D.seedPrograms().flatMap(raw=>{
      ...
      if(!M.canViewProgramResult(program,domain,viewer))return [];
      return [{program,sharing,detail,participant}];
    }).sort((a,b)=>dateKey(b.sharing.sharedAt).localeCompare(dateKey(a.sharing.sharedAt)));
  }
  function countFor(domain,viewer){return entriesFor(domain,viewer).length;}
  ...
  function canShowBadges(program){return program&&program.requestSource!=='hr';}
  function badgeSummary(participant,base,program){if(!canShowBadges(program))return '';const tally=model().coreValueTally(participant);return Object.entries(tally).map(([id,count])=>`<span class="employee-badge-item pms-tooltip" tabindex="0"><img src="${badgePath(base,id)}" alt=""/><span>${count}</span><span class="pms-tooltip-content">${BADGE_LABEL[id]||id}</span></span>`).join('');}
  function responseBadges(assignment,base,program){return !canShowBadges(program)?'':(assignment.badges||[]).map(id=>`<span class="pms-tooltip"><img class="cv-icon" src="${badgePath(base,id)}" alt=""/><span class="pms-tooltip-content">${BADGE_LABEL[id]||id}</span></span>`).join('');}
  ...
  function answerRecord(item,anon,base,program){const reviewer=item.assignment.reviewer,badges=responseBadges(item.assignment,base,program);return `<article class="question-response">${reviewerAvatar(reviewer,anon)}<div class="question-response-main"><div class="question-response-head"><div><div class="fb-line">${reviewerIdentity(reviewer,anon)}</div><div class="fb-date">${item.assignment.submittedAt}</div></div>${badges?`<div class="cv-icons">${badges}</div>`:''}</div><p class="fb-body">${item.answer.body}</p></div></article>`;}
  ...
  function evidenceGroup(group,index,single,anon,base,program){const count=group.answers.length,question=questionPrompt(group.question.text),head=single?`<div class="shared-question single-question"><p class="question-text">${question}</p></div>`:`<details class="question-disclosure" open><summary>${question}<span class="question-response-count">${count} phản hồi</span></summary></details>`;const body=count?group.answers.map(item=>answerRecord(item,anon,base,program)).join(''):`<div class="content-note"><i class="bx bx-message-square-dots"></i>Chưa có phản hồi cho câu hỏi này.</div>`;return `<section class="question-evidence-group">${head}<div class="question-answers">${body}</div></section>`;}
  ...
  function detailHTML(entry,opts){
    ...
    const badges=badgeSummary(entry.participant,base,entry.program);
    ...
    let body='';
    if(showsDetail(entry)){const groups=groupAnswers(entry),single=(entry.detail.questions||[]).length===1;body=`<div class="label">Phản hồi chi tiết</div>${groups.map((group,index)=>evidenceGroup(group,index,single,anon,base,entry.program)).join('')}`;}

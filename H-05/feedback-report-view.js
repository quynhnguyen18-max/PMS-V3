...
(function(root,factory){
  ...
  function evidenceGroup(group,index,single,anon,base){const count=group.answers.length,head=single?`<div class="shared-question"><div class="label">Câu hỏi</div><p class="question-text">${group.question.text}</p></div>`:`<details class="question-disclosure" open><summary><span class="question-disclosure-label">Câu ${index+1}</span><span class="question-response-count">${count} phản hồi</span></summary><p class="question-wording">${group.question.text}</p></details>`;const body=count?group.answers.map(item=>answerRecord(item,anon,base)).join(''):`<div class="content-note"><i class="bx bx-message-square-dots"></i>Chưa có phản hồi cho câu hỏi này.</div>`;return `<section class="question-evidence-group">${head}<div class="question-answers">${body}</div></section>`;}
